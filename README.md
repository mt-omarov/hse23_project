...
* [Тепловая карта](#map)
  - [Ссылка на ноутбук colab](https://colab.research.google.com/drive/1CIk0L_XlxL4Zhod7AN9P0Cen1UlIiZn4?usp=sharing)
* [Выводы](#result)

<a name = "setup"></a>
## Подготовка
Для изучения был выбран белок CBX1. \
Рассмотреть общую информацию, таргеты и функцию белка можно на сайте [epifactors](https://epifactors.autosome.org/genes/by_hgnc/CBX1). \
Далее важно определить fasta-последовательность белка, найти её можно по следующей [ссылке](https://www.ncbi.nlm.nih.gov/protein/NP_001120700.1?report=fasta). \
Подробно ознакомиться с белком можно [здесь](https://www.ncbi.nlm.nih.gov/gene/10951#reference-sequences). \
Определить домены можно на сайте [ncbi_domains](https://www.ncbi.nlm.nih.gov/Structure/cdd/wrpsb.cgi), указав полученную fasta-последовательность белка.
...
Для построения тепловой карты использовался данный [google colab ноутбук](https://colab.research.google.com/drive/1CIk0L_XlxL4Zhod7AN9P0Cen1UlIiZn4?usp=sharing). 

С помощью библиотек `pandas` и `numpy` обрабатываются значения экспортирумой в csv таблицы e-values, после чего с помощью `seaborn.sns` строится тепловая карта. \
Для наглядности результатов верхним значением (пороговым значением) при переводе в логарифмическую шкалу было выбрано число 100 (так разница "расстояний" более различима).

![image](https://github.com/mt-omarov/hse23_project/assets/95280619/4228011f-bd66-4b17-9b18-e7c97a11a6d0)

<a name = "result"></a>  
## Вывод
Из полученной тепловой карты можно заключить, что ген CBX1 впервые появился ещё у инфузорий и далее наблюдается у круглых червей caenorhabditis elegans. \
Отчётливо ген проявляется у хордовых и позвоночных.
